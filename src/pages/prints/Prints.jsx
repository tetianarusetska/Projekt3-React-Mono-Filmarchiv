...
export default function Prints({ className = "", gridClassName = "", cardClassName = "" }) {
    ...
    return (
        <div className={`grid ${className} ${gridClassName}`}>
            {photos.map(photo => (
                <div
                    key={photo.id}
                    className={`font-[Fejoya] rounded-2xl text-[18px] text-(--mainColor) backdrop-blur-lg border border-(--mainColor) cursor-pointer transition-all duration-300 lg:hover:-translate-y-2 hover:shadow-2xl ${cardClassName}`}
                    >
